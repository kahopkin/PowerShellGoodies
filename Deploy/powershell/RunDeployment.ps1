...
Function global:RunDeployment {
   ...
   #>
    ...
    if($AppRegObj.FileExists)
    {
        ...
        #Write-Host -ForegroundColor Green "RunDeployment[175] Finished setting the RedirectURI for $ApiAppRegName"
    ...
    {
        Write-Host -ForegroundColor Red "RunDeployment[87] calling ConfigureAPI, ConfigureWebClient !!!"   
                
        $ApiAppRegJson = ConfigureAPI -AppName $ApiAppRegName -AppRegObj $AppRegObj
        
        $AppRegObj = @{}
        $AppRegObj = $ApiAppRegJson 
        
        #Write-Host -ForegroundColor Cyan "RunDeployment[93] AppRegObj"
        #PrintObject -object $AppRegObj -Caller $Caller
        
        $ClientAppRegJson = ConfigureWebClient -AppName $AppName -AppRegObj $AppRegObj
        #$AppRegObj = $ClientAppRegJson
        $AppRegObj = @{}
        $AppRegObj = $ClientAppRegJson
...
Param(
    ...
    Write-Host -ForegroundColor Cyan "RunDeployment.ConfigureAPI[165] $AppName : $AppName Created"
   ...
   <# 
    ...
    if($ApiAppRegObj.ApiExisting -eq $false)
    {
        ...
        #Write-Host -ForegroundColor Green "RunDeployment.ConfigureAPI[183] Finished setting the RedirectURI"
        ...
        #Write-Host -ForegroundColor Yellow "ConfigureAPI[168] Spn.AppDisplayName:" $Spn.AppDisplayName
        ...
        Write-Host -ForegroundColor Yellow "RunDeployment.ConfigureAPI[178] AddAPIPermissions: $PermissionParentName.$RequiredDelegatedPermissionNames for $AppName"
        ...
        Write-Host -ForegroundColor Yellow "RunDeployment.ConfigureAPI[194] AddAPIPermissions: $PermissionParentName.$RequiredDelegatedPermissionNames for $AppName"
    ...
    Write-Host -ForegroundColor Green "RunDeployment.ConfigureAPI[253] $AppName Application's API Permissions are updated!"												   
    $today = Get-Date -Format "MM-dd-yyyy-HH-mm:ss"
...
Function global:ConfigureWebClient{
    ...
    {        
        #$SPN = CreateServicePrincipal -AppId $AppId
        #Write-Host -ForegroundColor Yellow "RunDeployment.ConfigureWebClient[286] Spn.AppDisplayName:" $Spn.AppDisplayName
        
        #SetRedirectURI -ObjectId $AppObjectId
        #Write-Host -ForegroundColor Green "RunDeployment.ConfigureWebClient[292] Finished setting the RedirectURI"
        ...
        # Add the permissions to the application
        Update-MgApplication -ApplicationId $AppObjectId -RequiredResourceAccess $RequiredResourcesAccessList
        Write-Host -ForegroundColor Green "RunDeployment.ConfigureWebClient[301] $AppName Application's API Permissions are updated!"

    }#clientObj[0].Existing -eq 'false'
    else
    {
        Write-Host -ForegroundColor Yellow "RunDeployment.ConfigureWebClient[306] clientObj[0].Existing: " $clientObj[0].Existing
...
Param(
    ...
    if (Test-Path $OutFileJSON) 
    {
        ...
        Write-Host "RunDeployment.IngestJsonFile[393] jsonFileName.FullPath: " $FullPath.FullName        
        Write-Host "RunDeployment.IngestJsonFile[394] File: $OutFileJSON Exists"

        $FileExists = $true
        $json = Get-Content $OutFileJSON | Out-String | ConvertFrom-Json
        Write-Host "RunDeployment.IngestJsonFile[397] json.Tenant:" $json.Tenant
    ...
    {
        Write-Host -ForegroundColor Red "!!RunDeployment.IngestJsonFile[441] $jsonFileName Doesn't Exists, Creating object"
        $object = @{
            Tenant = $Tenant
            TenantId = $TenantId
    ...
    $today = Get-Date -Format "MM-dd-yyyy-HH-mm:ss"
    Write-Host -ForegroundColor Magenta -BackgroundColor Black  "`n*** [$today] FINISHED IngestJsonFile ***`n"
    return $object
}#IngestJsonFile
