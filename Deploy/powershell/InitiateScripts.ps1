...
Function global:InitializeAzResourcesComplexObj
{
	...
	If($debugFlag)
	{
		...
		$Caller = "`n[" + $today + "] STARTING InitiateScripts.InitializeAzResourcesComplexObj[777]"
		Write-Host -ForegroundColor DarkBlue -BackgroundColor White $Caller
		<#
		$SolutionObjName = $Object.SolutionObjName
		Write-Host -ForegroundColor Yellow "`$SolutionObjName= " -NoNewline
		Write-Host -ForegroundColor Cyan "`"$SolutionObjName`""
		#>
	}#If($debugFlag)#>
...
Function global:SetDeployInfoObj
{
	...
	If($debugFlag){
		$today = Get-Date -Format "MM/dd/yyyy HH:mm:ss"
		Write-Host -ForegroundColor DarkBlue -BackgroundColor White "`n[$today] STARTING InitiateScripts.SetDeployInfoObj[390]"
	...
	Else
	{
		...
		#
		If($PrintPSCommands){
			Write-Host -ForegroundColor Magenta "InitiateScripts.SetDeployInfoObj[408]:"
		...
		$psCommand = "`$DefaultWorkspaces = `n`t`(Get-AzOperationalInsightsWorkspace  ```n`t`t" + 
...
{ 
	...
	If($debugFlag){
		$today = Get-Date -Format "MM/dd/yyyy HH:mm:ss"
		Write-Host -ForegroundColor DarkBlue -BackgroundColor White "`n[$today] STARTING InitiateScripts.SetDeployInfoPropsFromFile[415]"
	...
	If (Test-Path ($FilePath))
	{
		...
		#If($DeployObject.Solution -eq $null){ $DeployObject.Solution = $json.Solution;}
		...
		If($DeployObject.VirtualNetworkName -eq $null){$DeployObject.VirtualNetworkName = $json.VnetName;}
		...
		#WriteJsonFile -FilePath $FilePath -CustomObject $DeployObject 
		<#
		If($DeployObject.DebugFlag){
			#Write-Host -ForegroundColor Yellow "InitiateScripts.SetDeployInfoPropsFromFile[544] DeployObject.DebugFlag=" $DeployObject.DebugFlag
			$ObjectName = "DeployObject"
			$Caller ='InitiateScripts.SetDeployInfoPropsFromFile[546]: DeployObject AFTER Get-Content from ' + $FilePath
			PrintCustomObject -Object $DeployObject -Caller $Caller -ObjectName $ObjectName
		}#>
	}#If (Test-Path ($FilePath))
	Else
	{
...
Function global:ConfigureDeployInfo
{
	...
	If($debugFlag){
		$today = Get-Date -Format "MM/dd/yyyy HH:mm:ss"
		Write-Host -ForegroundColor DarkBlue -BackgroundColor White "`n[$today] STARTING InitiateScripts.ConfigureDeployInfo[672]"
	}#If($debugFlag)#>
	...
	{
		$DeployObject.AppName = GetAppName 
	}

	$DeployObject.AppName = $AzResourcesComplexObj.WebSite.ResourceName = (Get-Culture).TextInfo.ToTitleCase($DeployObject.AppName)
		
	If($DeployObject.DeployMode.Length -eq 0)
	{
		$DeployObject.DeployMode = PickDeployMode
	}

	$DeployObject.ResourceGroupName= "rg-"+ (Get-Culture).TextInfo.ToLower($DeployObject.AppName)  + "-" +
											#(Get-Culture).TextInfo.ToLower($DeployObject.Solution) + "-" +
											(Get-Culture).TextInfo.ToLower($DeployObject.Environment)
	...
	$SolutionNameSt = $DeployObject.AppName.toLower() + $DeployObject.Solution.toLower() + $DeployObject.Environment.toLower()
	#$DeployObject.TransferAppObj = InitializeTransferObject
	$DeployObject.SolutionObjName = "TransferAppObj"
	...
	{
		$DeployObject.KeyVaultName = $AzResourcesComplexObj.KeyVault.ResourceName = ("kv-" + $DeployObject.SolutionName).Substring(0,24)
	}
	Else
	{
		$DeployObject.KeyVaultName = $AzResourcesComplexObj.KeyVault.ResourceName = "kv-" + $DeployObject.SolutionName
	}
	...
	If( ("staudit" + $SolutionNameSt + "00").length -gt 23 )
	{
		$DeployObject.AuditStorageName =
		$AzResourcesComplexObj.AuditStorage.ResourceName = ("staudit" + $SolutionNameSt + "00").Substring(0,24)
	}
	Else
	{
		$DeployObject.AuditStorageName =
		$AzResourcesComplexObj.AuditStorage.ResourceName = "staudit" + $SolutionNameSt + "001"
	}

	If( ("st" + $SolutionNameSt + "001").length -gt 23 )
	{
		$DeployObject.MainStorageName =
		$AzResourcesComplexObj.MainStorage.ResourceName = ("st" + $SolutionNameSt + "001").Substring(0,24)
	}
	Else
	{
		$DeployObject.MainStorageName =
		$AzResourcesComplexObj.MainStorage.ResourceName = "st" + $SolutionNameSt + "001"
	}
	...
	If($debugFlag){
		...
		Write-Host -ForegroundColor Cyan "Solution= " $DeployObject.Solution
...
Function global:ConfigureTransferAppObj
{
	...
	If($debugFlag){
		$today = Get-Date -Format "MM/dd/yyyy HH:mm:ss"
		Write-Host -ForegroundColor DarkBlue -BackgroundColor White "`n[$today] STARTING InitiateScripts.ConfigureTransferAppObj[677]" 
	}#If($debugFlag)#>
	Else{
		$Message = "SETTING SPECIFIC OBJECT PROPERTIES"
		$DeployObject.StepCount = PrintMessage -Message $Message -StepCount $DeployObject.StepCount
		#PrintMessageToFile -Message $Message -StepCount $DeployObject.StepCount -LogFile $DeployObject.LogFile
	}
			

	$DeployObject.Solution = "Transfer"
	$AppName = $DeployObject.AppName
	If( $DeployObject.Environment.ToLower() -ieq 'prod')
	{
		If( $AppName.ToLower() -match '^dtp')
		{ 
			$AppName = $AppName
		}
		ElseIf($AppName.ToLower() -match '^dts')
		{
			$AppName = $AppName + $DeployObject.Solution
			}
		Else
		{
			$AppName = $AppName + $DeployObject.Solution + $DeployObject.Environment
		}
	}
	Else
	{ 
		If( ($AppName.ToLower() -match '^dtp') )
			{ 
			$AppName = $AppName + $DeployObject.Environment
		} 
			Else
		{
			$AppName = $AppName + $DeployObject.Solution + $DeployObject.Environment
	...
	$AzResourcesComplexObj.FunctionApp.ResourceName = $AppName + 'API'
	$DeployObject.ClientAppRegName =
	$AzResourcesComplexObj.WebSite.ResourceName = $AppName
	If($DeployObject.SqlAdmin.Length -eq 0)
	{
		 $DeployObject.SqlAdmin = $DeployObject.SqlAdmin = Read-Host "Enter SQL Server Admin Login" #-AsSecureString }
	}
	...
	If($DeployObject.SqlAdminPwd.Length -eq 0)
	{
		...
		$DeployObject.SqlAdminPwd = $SqlAdminPwdPlainText
	}
	<#
	Else
	{
			$DeployObject.SqlAdminPwd = $DeployObject.SqlAdminPwd 
	}
	#>
