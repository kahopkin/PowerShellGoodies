Function global:CreateEnvironmentFiles
{
	...
	Write-Host -ForegroundColor Cyan "Step" $DeployObject.StepCount ": CREATE .ENV FILES:" 
	Write-Host -ForegroundColor Cyan "================================================================================"
	$DeployObject.StepCount++

	If($debugFlag)
	{
		Write-Host -ForegroundColor Magenta "CreateEnvironmentFiles[19]:"
	...
	$DPPEnvObject = [ordered]@{
		...
		REACT_APP_AAD_AUTHORITY = $DeployObject.Cloud.ActiveDirectoryAuthority + $DeployObject.TenantId;
		REACT_APP_AAD_REDIRECT_URI = "https://" + $DeployObject.ClientAppRegName + ".azurewebsites.us";
		REACT_APP_LOGIN_SCOPES = "array:User.Read";
		#REACT_APP_GRAPH_ENDPOINT = $DeployObject.Cloud.GraphUrl + "v1.0/me";
	...
	If($debugFlag)
						...
						If($DirectoryName -eq "DPP" -and $DeployObject.Solution -ne "Transfer")
						{
							...
							Switch($FileName)
							{
									...
									If($debugFlag)
									{
										Write-Host -ForegroundColor Cyan "[194] DPP: .env "
...
Function global:ProcessFile
{
	...
	Write-Host -ForegroundColor Yellow "[$today] START Writing the .env file at:"
	 Write-Host -ForegroundColor Green $EnvFile
	Write-Host -ForegroundColor Yellow "================================================================================"
