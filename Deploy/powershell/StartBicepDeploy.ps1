...
Function global:StartBicepDeploy {
    ...
    if($debugFlag)
    {        
        Write-Host -ForegroundColor White -BackgroundColor Black  "StartBicepDeploy[51] BICEP Command: "
        ...
        New-AzSubscriptionDeployment `
            ...
            $today = Get-Date -Format "MM-dd-yyyy-HH-mm:ss"
            Write-Host -ForegroundColor Yellow -BackgroundColor Blue  "StartBicepDeploy[93] **** FINISHED AzSubscriptionDeployment *****"
    ...
    {
        Write-Host -ForegroundColor White -BackgroundColor Black  "StartBicepDeploy[109] BICEP Command: "
        Write-Host -ForegroundColor Cyan "New-AzSubscriptionDeployment `
            -Name $DeploymentName ```
            -Location $Location ```
            -TemplateFile $templateFile ```
        ...
        New-AzSubscriptionDeployment `
            ...
            $today = Get-Date -Format "MM-dd-yyyy-HH-mm:ss"
            Write-Host -ForegroundColor Yellow -BackgroundColor Blue  "StartBicepDeploy[131] **** FINISHED AzSubscriptionDeployment *****"
            Write-Host -ForegroundColor Green -BackgroundColor Black  "`n*************[$today]: EXITING StartBicepDeploy Function *****************" 
            return $ApiClientId
            }
        catch{            
