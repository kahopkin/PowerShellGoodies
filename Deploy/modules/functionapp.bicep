...
resource apiFunctionApp_AppSettings_Transfer 'Microsoft.Web/sites/config@2021-03-01' = if(Solution == 'Transfer') {
  parent: functionApp
  name: 'appsettings'
  properties: (Solution == 'Transfer') ? {        
    APPINSIGHTS_INSTRUMENTATIONKEY:'@Microsoft.KeyVault(VaultName=${keyVault.name};SecretName=APPINSIGHTS_INSTRUMENTATIONKEY)'    
    APPLICATIONINSIGHTS_CONNECTION_STRING: '@Microsoft.KeyVault(VaultName=${keyVault.name};SecretName=APPLICATIONINSIGHTS_CONNECTION_STRING)'
...
/*
resource apiFunctionApp_AppSettings_Pickup 'Microsoft.Web/sites/config@2021-03-01' = if(Solution == 'Pickup') {
  parent: functionApp
  name: 'appsettings'
  properties: {    
    applyRBACTimerInterval: (Solution == 'Transfer') ? '' : '0 00 * * * *'
    APPINSIGHTS_INSTRUMENTATIONKEY:'@Microsoft.KeyVault(VaultName=${keyVault.name};SecretName=APPINSIGHTS_INSTRUMENTATIONKEY)'    
    APPLICATIONINSIGHTS_CONNECTION_STRING: '@Microsoft.KeyVault(VaultName=${keyVault.name};SecretName=APPLICATIONINSIGHTS_CONNECTION_STRING)'
    armTemplateQueueName: (Solution == 'Transfer') ? 'deployarmtemplates' : ''
    authEndpoint: environment().authentication.loginEndpoint    
    AuditStorageAccessKey: (Solution == 'Transfer') ? '@Microsoft.KeyVault(VaultName=${keyVault.name};SecretName=AuditStorageAccessKey)' : ''    
    AzStorageAccessKey: '@Microsoft.KeyVault(VaultName=${keyVault.name};SecretName=AzStorageAccessKey)' 
    AzureWebJobsSecretStorageType: 'blob'    
    AzureWebJobsStorage: '@Microsoft.KeyVault(VaultName=${keyVault.name};SecretName=AzureWebJobsStorage)' 
    blobEndpoint: 'https://${mainStorageAccount.name}.blob.${environment().suffixes.storage}'    
    ...
    clientSecret: '@Microsoft.KeyVault(VaultName=${keyVault.name};SecretName=clientSecret)' 
    completedContainerName: 'completedcontainers'    
    createCTSContainer: (Solution == 'Transfer') ? 'false' :''
    ctsStorageSASUri: (Solution == 'Transfer') ? connectionStringSAS :'' 
    deleteAuditBlobContainerName:  (Solution == 'Transfer') ? 'insights-logs-storagedelete' : ''
    FUNCTIONS_WORKER_RUNTIME: functionRuntime
    FUNCTIONS_EXTENSION_VERSION: '~4'    
    MICROSOFT_PROVIDER_AUTHENTICATION_SECRET: ApiClientSecret 
    ftpState: 'Disabled'
    readAuditBlobContainerName: (Solution == 'Transfer') ? 'insights-logs-storageread' : ''
    roleDefinitionId:RoleDefinitionId
    sentinelTimer: (Solution == 'Transfer') ? '0 59 23 * * *': ''     
    SqlConnectionString: (Solution == 'Transfer') ? '@Microsoft.KeyVault(VaultName=${keyVault.name};SecretName=SqlConnectionString)' : ''       
    storageAccountResourceID: '@Microsoft.KeyVault(VaultName=${keyVault.name};SecretName=storageAccountResourceID)'
    subscriptionID: subscriptionID
    tenantID: tenantId  
    transfersTableName: (Solution == 'Transfer') ? 'transfers': 'CompletedContainers'
    transferCompilationRequestsQueueName: (Solution == 'Pickup') ? 'compilationrequests' : ''
    transferValidationQueueName : (Solution == 'Transfer') ? 'status': ''    
    validationTimeOut: (Solution == 'Transfer') ?  '10' :''
    WEBSITE_CONTENTOVERVNET:'1'    
    WEBSITE_RUN_FROM_PACKAGE: '1'
    WEBSITE_DNS_SERVER: '168.63.129.16'
    //WEBSITE_CONTENTSHARE: toLower(FunctionAppName) 
    //WEBSITE_CONTENTSHARE:contentShare    
    //WEBSITE_VNET_ROUTE_ALL : '1'    
    writeAuditBlobContainerName:(Solution == 'Transfer') ?  'insights-logs-storagewrite' : ''
    XDT_MicrosoftApplicationInsights_Mode: 'Recommended'
    applyRBACTimerInterval: (Solution == 'Transfer') ? '' : '0 00 * * * *'   
  }
}
