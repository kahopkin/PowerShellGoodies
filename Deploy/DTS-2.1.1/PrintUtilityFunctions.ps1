...
Function global:PrintLogInfo
{
	...
	{
		$Message = $Message + "DPP Custom Role Definition file:`n"
		$Message = $Message + $DeployObject.PickupAppObj.RoleDefinitionFile + "`n"
	}
	$Message = $Message + $Dashes
...
Function global:PrintDeploymentOutput{
	...
	If($debugFlag){
		$today = Get-Date -Format "MM/dd/yyyy HH:mm:ss"
		Write-Host -ForegroundColor DarkBlue -BackgroundColor White "`n[$today] DTS-2.1 STARTING PrintUtitlityFunctions.PrintDeploymentOutput[582]"
		Write-Host -ForegroundColor DarkBlue -BackgroundColor White "`$Caller= `"$Caller`""
	}
	...
	ForEach ($item in $Object.Keys)
	{
		...
		#Write-Host -ForegroundColor Yellow "`$itemType=" -NoNewline
...
Function global:PrintDeployDuration
{
	...
	"[$today] COMPLETED DEPLOYMENT " + $DeployObject.Solution		>> $DeployObject.LogFile
